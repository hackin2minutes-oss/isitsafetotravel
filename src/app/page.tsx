...
type ViewMode = 'location' | 'global' | 'compare' | 'comms';
...
export default function Home() {
  ...
  const [selectedCountry, setSelectedCountry] = useState<ScoredCountry | null>(null);
  ...
  // --- RENDER HELPERS ---
  const renderGlobalIndex = () => (
    <div className="space-y-4 animate-fade-up">
      <div className="relative glass-panel rounded-2xl p-4 overflow-hidden group">
        <div className="flex items-center gap-3 relative z-10">
          ...
          <div>
            <h2 className="text-sm font-black text-white tracking-widest uppercase mb-0.5">Global Status Index</h2>
            <p className="text-[7px] font-mono-technical text-slate-500 uppercase">Live Fed // Sector Sync</p>
          </div>
        </div>
      </div>

      <div className="relative group">
        <div className="absolute left-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-600 transition-colors" />
        <input
          type="text"
          placeholder="FILTER SECTORS..."
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
          className="w-full bg-white/[0.02] border border-white/10 rounded-xl pl-10 pr-4 py-3 text-[10px] font-black text-white placeholder:text-slate-700 tracking-widest uppercase focus:outline-none focus:border-indigo-500/30 transition-all font-mono"
        />
        <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-600" />
      </div>

      <div className="grid grid-cols-1 gap-2 max-h-[50vh] overflow-y-auto pr-1 subtle-scroll scrollbar-hide">
        {filteredCountries.slice(0, 40).map((country) => {
          ...
          return (
            <button
              ...
              className={`w-full group relative overflow-hidden rounded-2xl p-4 transition-all duration-300 border ${
                isSelected 
            ...
            >
              <div className="flex items-center gap-4 relative z-10">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-[10px] shrink-0 border ${getRankBg(country.rank || 1)} ${getRankColor(country.rank || 1)}`}>
                  #{country.rank}
                </div>
                <div className="flex-1 min-w-0 text-left">
                  <p className="font-black text-white group-hover:neon-text transition-all tracking-tight uppercase text-sm">{country.name}</p>
                  <p className="text-[10px] font-bold text-slate-500 tracking-widest uppercase">{country.code}</p>
                </div>
                <div className="text-right shrink-0">
                  <div className={`text-2xl font-black ${getRankColor(country.rank || 1)} tracking-tighter`}>
                    {country.score?.score?.toFixed(1)}
                  </div>
                  <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest">{rating.category}</span>
                </div>
              </div>
            </button>
          );
  ...
  return (
    <div className={`flex flex-col h-screen w-full overflow-hidden selection:bg-violet-500/30 bg-[#08090C] ${viewPreference !== 'auto' ? 'border-4 border-violet-500/20' : ''}`}>
        ...
        >
          ...
          <div className="p-6 flex items-center justify-between border-b border-white/[0.03]">
             <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">INTELLIGENCE UNIT</span>
             </div>
            ...
            <div className="relative p-1 bg-white/[0.02] border border-white/[0.05] rounded-xl flex items-center">
              <div 
                ...
                style={{
                  ...
                  transform: viewMode === 'location' ? 'translateX(0)' : viewMode === 'global' ? 'translateX(100.5%)' : viewMode === 'compare' ? 'translateX(201%)' : 'translateX(301.5%)'
                }}
              />
              ...
              <button onClick={() => setViewMode('global')} className={`relative z-10 flex-1 py-2.5 text-[9px] font-black uppercase tracking-widest transition-colors ${viewMode === 'global' ? 'text-indigo-400' : 'text-slate-500 hover:text-slate-300'}`}>Index</button>
              <button onClick={() => setViewMode('compare')} className={`relative z-10 flex-1 py-2.5 text-[9px] font-black uppercase tracking-widest transition-colors ${viewMode === 'compare' ? 'text-indigo-400' : 'text-slate-500 hover:text-slate-300'}`}>Strategic</button>
              <button onClick={() => setViewMode('comms')} className={`relative z-10 flex-1 py-2.5 text-[9px] font-black uppercase tracking-widest transition-colors ${viewMode === 'comms' ? 'text-indigo-400' : 'text-slate-500 hover:text-slate-300'}`}>Comms</button>
            </div>
          </div>
          ...
          <div className={`flex-1 overflow-y-auto px-6 pb-20 md:pb-8 subtle-scroll scrollbar-hide`}>
            ...
            {viewMode === 'global' && renderGlobalIndex()}
      ...
      {/* FOOTER */}
      <footer className="shrink-0 bg-[#08090C] border-t border-white/[0.03] py-4 px-6 flex items-center justify-between text-[8px] font-black text-slate-600 uppercase tracking-[0.3em]">
        <span>Operational Environment Beta v.1.0</span>
        <div className="flex gap-4">
          <span className="hover:text-white transition-colors cursor-pointer">Protocol</span>
          <span className="hover:text-white transition-colors cursor-pointer">Signal Status</span>
